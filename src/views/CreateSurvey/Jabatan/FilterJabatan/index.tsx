...
const FilterJabatan = ({
	...
	const fitleredData = useMemo(
		() => dataJabatan.filter((item) => item.label.includes(search)),
		[search],
	);
	...
	const handleCheckAll = () => {
		...
		if (!isCheckAll) {
			...
			const unique = [...new Map(allData.map((item) => [item[key], item])).values()];
	...
	return (
		<JabatanContainer>
			...
			>
				<label htmlFor="all" style={{ fontWeight: "500", fontSize: "14px" }}>
					Jabatan
				</label>
				<Checkbox id="all" checked={isCheckAll} onChange={handleCheckAll} />
			</Grid>
			<Grid flexDirection="column" gap={12} container style={{ padding: "10px 10px 8px 8px" }}>
				<Input
				...
				{fitleredData.map((item, idx) => (
					<Grid key={idx} container justifyContent="space-between">
						<label htmlFor={item.label} style={{ fontSize: "12px" }}>
							{item.label}
						</label>
						<Checkbox
							id={item.label}
							onChange={() => handleDataChecked(item.label, item.id)}
							checked={Boolean(dataChecked.find((data) => data.id == item.id))}
							value={item.id}
						/>
					</Grid>
				))}
