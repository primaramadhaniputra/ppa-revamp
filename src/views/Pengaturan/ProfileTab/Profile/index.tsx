...
export default function Profile() {
   ...
   useEffect(() => {
      getProfile({ path: 'profile' }).then(e => setUserName(e.data.data.fullName))
   }, [])
   ...
   const handleSubmitUpdateProfile = async (e: { preventDefault: () => void }) => {
      try {
         e.preventDefault()
         await axios.all([
            updateProfile({
   ...
   return (
      <Wrapper onSubmit={handleSubmitUpdateProfile}>
         ...
         <LabeledInput name='Nama-lengkap' title='Nama Lengkap' style={{ backgroundColor: colors.blueSky }} value={userName} onChange={handleFullNameChange} />
         <ButtonWrapper >
